...
vec = TfidfVectorizer(ngram_range=(1,2), max_df=0.9, use_idf=1,
...
preds = np.zeros((len(test), len(labels_classes)))

for i, j in enumerate(labels_classes):
    print('fit', j)
    m,r = get_mdl(train[j])
...
submission = pd.concat([submid, pd.DataFrame(preds, columns = labels_classes)], axis=1)
submission.to_csv('/Users/amitn/Documents/kaggle/toxicity/nbsvm_submission.csv', index=False)
